...
one_hot <- read_tsv("GDM_analysis/trial/one_hot_encoded_metadata.tsv")
all_meta <- read_tsv("decontamination/input/GCMP_EMP_map.txt")
meta.comb <- (merge(one_hot, all_meta, on='SampleID'))

#write it as a tsv so that we can then reload it as a phyloseq object
write_tsv(meta.comb, "GDM_analysis/one_hot/input/combined_meta.tsv")
...
physeq <- qza_to_phyloseq("decontamination/output/physeq.noncont-feature-table.qza", #feature table
                          "decontamination/output/physeq.noncton-rooted-tree.qza", #tree
                          "organelle_removal/output/silva_metaxa2_reference_taxonomy.qza", #taxonomy reference
                          "GDM_analysis/one_hot/input/combined_meta.tsv") #mapping file
...
metadata_ord_filt <- metadata_ord %>% filter(temperature != "Missing: Not collected" &
...
source("GDM_analysis/trial/jld_gdm_helpers.r")
