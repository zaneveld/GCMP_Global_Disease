...
```{r}
phylo.heatmap(tree, X, standardize=FALSE, fsize=0.6, colors=disease_ramp_color_palette,length=1)
```
```{r}
...
```{r}
phylomorphospace(ctree, phy, colors=cols, node.size=1.25, node.by.map=TRUE, fsize=0.01, label="off")
